...
- **train_BERT.py**  
  ...
  The dataset itself does not explicitly label models as "strong" or "weak," so two predefined lists at the beginning of the script map specific models into these categories. The resulting model is stored in `models/router_arena`.

- **svm.py, knn.py, knn_arena.py**  
  Train alternative routing models (SVM or KNN) to predict when a large model is required.  
  The `_arena` suffix indicates that training was performed on the Arena dataset. All results are stored in `models`.
